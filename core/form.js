...
function addForm(x) { var that=this; var vObj=x.obj; var sId=vObj.id; var sAction=vObj.getAttribute('data-url');
 var sMethod=x.props['method']; var sTarget=x.props['target']; var sEncType=x.props['enctype']; var sOnReset=x.props['onreset'];
 var sOnSubmit=x.props['onsubmit']; var sName=x.props['name']; var sCustomCode=x.props['customcode'];
 var sRecipient=x.props['recipient']; var sSuccessPage=x.props['successpage']; var sErrorPage=x.props['errorpage'];
 var sTemplate=x.props['template']; var vLayer=vObj.parentNode; var vForm=document.createElement('form');
 if (sAction) { vForm.setAttribute('action',sAction); } if (sTarget) { vForm.setAttribute('target',sTarget); }
 if (sMethod) { vForm.setAttribute('method',sMethod); } if (sEncType) { vForm.setAttribute('enctype',sEncType); }
 if (sOnReset) { vForm.setAttribute('onreset',sOnReset); } if (sOnSubmit) { vForm.setAttribute('onsubmit',sOnSubmit); }
 if (sName) { vForm.setAttribute('name',sName); } if (sSuccessPage) { appendHiddenInput(vLayer,'successpage__',sSuccessPage); }
